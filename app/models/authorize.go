...
//根据Id获取信息
func (u *Authorize) GetById(Id int64) *Authorize {
...
//邮箱是否已有
func (u *Authorize) HasEmail() bool {
...
//获取会员列表
func (u *Authorize) GetUserList(search string, Page int64, Perpage int64) (user_arr []*Authorize, html template.HTML, where map[string]interface{}) {
	//初始化菜单
	user_list := []*Authorize{}
	...
	//查询总数
	user := new(Authorize)
	Total, err := DB_Read.Table("authorize").Where(WhereStr).Count(user)
	if err != nil {
		revel.WARN.Printf("错误: %v", err)
	}
...
//添加会员
func (u *Authorize) Edit(Id int64) bool {
	user := new(Authorize)
...
//删除用户
func (u *Authorize) DelByID(Id int64) bool {
...
//批量删除
func (u *Authorize) DelByIDS(ids string) bool {
	user := new(Authorize)
...
//锁定
func (u *Authorize) Lock(ids string) bool {
	user := new(User)
...
//解锁
func (u *Authorize) Unlock(ids string) bool {
	user := new(User)
...
//移动
func (u *Authorize) Move(groupid int64, ids string) bool {
	user := new(Authorize)
